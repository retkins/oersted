...
mod tests {
    ...
    fn test_mag() {
        ...
        let mvector = Vec3([1e7, 2e7, 3e7]);
        let mut h_exact = ([0.0], [0.0], [0.0]);

        let mut wx = [Vec3::default()];
        let mut wy = [Vec3::default()];
        let mut wz = [Vec3::default()];

        let target_nodes_pos = (
            &[1.8, 2.2, 2.0, 2.0][..],
            &[2.0, 2.0, 2.2, 2.0][..],
            &[2.0, 2.0, 2.0, 2.2][..],
        );
        let target_nodes_vec = [
            Vec3([1.8, 2.0, 2.0]),
            Vec3([2.2, 2.0, 2.0]),
            Vec3([2.0, 2.2, 2.0]),
            Vec3([2.0, 2.0, 2.2]),
        ];
        let target_conn = [[0u32, 1, 2, 3]];
        let j_invt_target = jmatrices(&target_nodes_vec, &target_conn);
        let mut f = vec![Vec3::default(); 4];
        let mut h_nodal = (vec![0.0; 1], vec![0.0; 1], vec![0.0; 1]);
        ...
        hmag_tet4_grad(
            ...
            &target_conn,
            &mut f,
            (&mut h_nodal.0, &mut h_nodal.1, &mut h_nodal.2),
        );
        h_mag_tet4(
            &nodes,
            &mvector,
            ...
            (&mut h_exact.0, &mut h_exact.1, &mut h_exact.2),
        );

        println!("{} {}", h_nodal.0[0], h_exact.0[0]);
        assert!(((h_nodal.0[0] - h_exact.0[0]) / h_exact.0[0]).abs() < 8e-2);
        assert!(((h_nodal.1[0] - h_exact.1[0]) / h_exact.1[0]).abs() < 8e-2);
        assert!(((h_nodal.2[0] - h_exact.2[0]) / h_exact.2[0]).abs() < 8e-2);
    }
}
